...
export default function AuthPage() {
  ...
  const submitSetup = async (event) => {
    ...
    try {
      ...
      setMode('login')
      setLoginData({ user_id: '', password: setupData.password })
  ...
  return <main className="auth-shell">
    <section className="auth-brand">
      ...
      <div className="trust-list">
        ...
        <span><KeyRound size={18} /> Unique user keys</span>
    ...
    <section className="auth-card-wrap">
      <div className="auth-card">
        ...
        <div className="auth-tabs">
          ...
          <button className={!isLogin ? 'selected' : ''} onClick={() => { setMode('setup'); setError('') }}>First-time setup</button>
        </div>
        {error && <div className="alert alert-error" role="alert">{error}</div>}
        {isLogin ? <form onSubmit={submitLogin} className="auth-form">
          <label>User key<input autoFocus name="user_id" value={loginData.user_id} onChange={change(setLoginData, loginData)} placeholder="e.g. USR_A1B2C3D4E5" required /></label>
          <label>Password<input name="password" type="password" value={loginData.password} onChange={change(setLoginData, loginData)} placeholder="Enter your password" required /></label>
          ...
          <label>Email address<input name="email" type="email" value={setupData.email} onChange={change(setSetupData, setupData)} required /></label>
